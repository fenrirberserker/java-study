...
public class GreedyBasics {
    ...
    /* ----------------------------------------------------------------- */
    /**
     * Returns the number of coins used to make `amount` by always picking the
     ...
     */
    public static int coinChangeGreedy(int amount, int[] coinsDesc) {
        Arrays.sort(coinsDesc);                // sort ascending
        int count = 0;
        for (int i = coinsDesc.length - 1; i >= 0; i--) {  // walk from largest
            int c = coinsDesc[i];
            count += amount / c;               // GREEDY: take as many as possible
            amount %= c;                       // remainder for the next coin
        }
        return count;
    }
